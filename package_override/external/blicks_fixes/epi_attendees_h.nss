...
void SetOtherNPCsAttending()
{
    ...
    // Shianni
    int         bShianni1 = WR_GetPlotFlag( PLT_DENPT_SLAVE_TRADE, DEN_SLAVE_TRADE_PC_FREED_SLAVES_IN_COMPOUND );
    int         bShianni2 = WR_GetPlotFlag( PLT_CLI600PT_ELVES, CLI_ELVES_ELDER_GIVE_REWARD_1 );
    int         bShianni3 = WR_GetPlotFlag( PLT_BEC000PT_MAIN, BEC_MAIN_VAUGHAN_DEAD );
    object      oShianni = UT_GetNearestCreatureByTag( oPC, EP1_CR_SHIANNI );
    if(( bShianni1 || bShianni2 || bShianni3 ) || bALLNPCs)
    ...
    // Soris
    int         bSoris1 = WR_GetPlotFlag( PLT_DENPT_RESCUE_THE_QUEEN, DEN_RESCUE_FREED_SORRIS );
    int         bSoris2 = WR_GetPlotFlag( PLT_BEC100PT_SORIS, BEC_SORIS_SAVED );
    object      oSoris = UT_GetNearestCreatureByTag( oPC, EPI_CR_SORIS );
    if(( bSoris1 || bSoris2 ) || bAllNPCs)
    ...
    // Wulff
    int         bWulff = WR_GetPlotFlag( PLT_DENPT_MAIN, LANDSMEET_PERSUADED_WULFF );
    object      oWulff = UT_GetNearestCreatureByTag( oPC, EPI_CR_WULFF );
    if(( bWulff ) || bAllNPCs)
