...
void main() {
    ...
    if (sAreaTag == "str100ar_hut_exterior") {   
        /* Run the Awakening initialisation code
        *  This should only be run once, but may be run from one of several scripts so it has it's own varaible */
        if (!AF_IsModuleFlagSet(AF_GENERAL_FLAG, AF_GENERAL_AWAKENING_INIT)) {
            ...
            object oItem = GetItemPossessedBy(oHero, "prc_dao_lel_im_arm_cht_01");
