...
void _HandleImpact(struct EventSpellScriptImpactStruct stEvent) {
    ...
    eEffect = Effect(AF_EFFECT_TYPE_STONE_WILL);
    ApplyEffectOnObject(EFFECT_DURATION_TYPE_TEMPORARY, eEffect, stEvent.oCaster, 20.0, stEvent.oCaster, stEvent.nAbility);

    // bonus physical/mental resistance
    eEffect = EffectModifyProperty(PROPERTY_ATTRIBUTE_RESISTANCE_PHYSICAL, fResist, PROPERTY_ATTRIBUTE_RESISTANCE_MENTAL, fResist);
    ApplyEffectOnObject(EFFECT_DURATION_TYPE_TEMPORARY, eEffect, stEvent.oCaster, 20.0, stEvent.oCaster, stEvent.nAbility);
...
void main()
{
    ...
    switch(nEventType) {
        ...
        case EVENT_TYPE_SHALE_STONE_WILL: { // heartbeat effect, resets movement speed
            ...
            if (GetHasEffects(oCaster, AF_EFFECT_TYPE_STONE_WILL, AF_ABILITY_STONE_WILL)) {
                ...
                if (GetArraySize(eBuff) > 1 || GetArraySize(eDebuff) > 0) {
                    ...
                    ApplyEffectOnObject(EFFECT_DURATION_TYPE_TEMPORARY, eEffect, oCaster, 20.0, oCaster, AF_ABILITY_STONE_WILL);
                }
