...
#include "af_effects_h"

const int SHALE_RANGED_MODE_AOE = 2003;
const int SHALE_TAUNT_AURA = 2010;

// different radii for Stone Aura
const int AF_ABILITY_STONE_AURA_1 = 2011; // 6m
const int AF_ABILITY_STONE_AURA_2 = 2012; // 6.5m
const int AF_ABILITY_STONE_AURA_3 = 2013; // 7m
const int AF_ABILITY_STONE_AURA_4 = 2014; // 8m
...
void _ActivateModalAbility(struct EventSpellScriptImpactStruct stEvent)
{
    ...
    switch (stEvent.nAbility) {
        case AF_ABILITY_PULVERIZING_BLOWS: {
            ...
            if (HasAbility(stEvent.oCaster, AF_ABILITY_SLAM)) {
                fDamageInc += 2.0;
                fAttackInc += 2.0;
                fAPInc += 3.0;
            }

            if (HasAbility(stEvent.oCaster, AF_ABILITY_QUAKE)) {
                fDefDec -= 5.0; // total: -15
            }

            if (HasAbility(stEvent.oCaster, AF_ABILITY_KILLING_BLOW)) {
            ...
            eEffects[nIndx] = EffectModifyProperty(PROPERTY_ATTRIBUTE_DAMAGE_BONUS, fDamageInc, PROPERTY_ATTRIBUTE_ATTACK, fAttackInc);
            eEffects[nIndx++] = SetEffectEngineInteger(eEffects[0], EFFECT_INTEGER_VFX, nVFX); // VFX - looping glow
            eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_DEFENSE, fDefDec, PROPERTY_ATTRIBUTE_ARMOR, fArmorDec);
        ...
        case AF_ABILITY_STONEHEART: {
            ...
            if (HasAbility(stEvent.oCaster, AF_ABILITY_BELLOW)) {
                fArmorInc += 3.0;
                fElResist += 6.0;
                fPhysResist += 8.0;
            }

            if (HasAbility(stEvent.oCaster, AF_ABILITY_REGENERATING_BURST)) {
            ...
            eEffects[nIndx] = EffectModifyProperty(PROPERTY_ATTRIBUTE_ARMOR, fArmorInc, PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_FIRE, fElResist, PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_COLD, fElResist);
            eEffects[nIndx++] = SetEffectEngineInteger(eEffects[0], EFFECT_INTEGER_VFX, nVFX); // VFX - looping glow

            eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_ELEC, fElResist, PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_NATURE, fElResist, PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_SPIRIT, fElResist);

            eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_RESISTANCE_PHYSICAL, fPhysResist, PROPERTY_ATTRIBUTE_RESISTANCE_MENTAL, fPhysResist);

            eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_DAMAGE_BONUS, fDamageDec, PROPERTY_ATTRIBUTE_MELEE_CRIT_MODIFIER, fCritChance);

            if (HasAbility(stEvent.oCaster, AF_ABILITY_STONE_WILL)) { // HP/stamina regen
                eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_REGENERATION_HEALTH, 3.0, PROPERTY_ATTRIBUTE_REGENERATION_HEALTH_COMBAT, 3.0);
                eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_REGENERATION_STAMINA, 2.0, PROPERTY_ATTRIBUTE_REGENERATION_STAMINA_COMBAT, 2.0);
            }
        ...
        case AF_ABILITY_ROCK_MASTERY: {
            ...
            if (HasAbility(stEvent.oCaster, AF_ABILITY_HURL_ROCK)) {
                fMissileDef += 10.0;
            }
            ...
            if (HasAbility(stEvent.oCaster, AF_ABILITY_ROCK_BARRAGE)) {
                fMissileDef += 10.0; // total: 70
            }

            eEffects[nIndx] = EffectModifyProperty(PROPERTY_ATTRIBUTE_MISSILE_SHIELD, fMissileDef, PROPERTY_ATTRIBUTE_ATTACK, fAttackDec);
            eEffects[nIndx++] = SetEffectEngineInteger(eEffects[0], EFFECT_INTEGER_VFX, nVFX); // VFX - looping glow
        ...
        case AF_ABILITY_STONE_AURA: {
            ...
            if (HasAbility(stEvent.oCaster, AF_ABILITY_SUPERNATURAL_RESILIENCE)) {
                nID = AF_ABILITY_STONE_AURA_4; // 8m
            } else if (HasAbility(stEvent.oCaster, AF_ABILITY_RENEWED_ASSAULT)) {
                nID = AF_ABILITY_STONE_AURA_3; // 7m
            } else if (HasAbility(stEvent.oCaster, AF_ABILITY_INNER_RESERVES)) {
                nID = AF_ABILITY_STONE_AURA_2; // 6.5m
            } else {
                nID = AF_ABILITY_STONE_AURA_1; // 6m
            }

            if (HasAbility(stEvent.oCaster, AF_ABILITY_INNER_RESERVES)) {
                fStamDec += 3.0;
            }

            if (HasAbility(stEvent.oCaster, AF_ABILITY_RENEWED_ASSAULT)) {
                fArmorInc += 5.0; // 20
                fElemResist += 10.0;  // 25
                fSpellResist += 8.0; // 20
            }
            ...
            eEffects[nIndx] = EffectModifyProperty(PROPERTY_ATTRIBUTE_ARMOR, fArmorInc, PROPERTY_ATTRIBUTE_SPELLRESISTANCE, fSpellResist, PROPERTY_ATTRIBUTE_RESISTANCE_PHYSICAL, fPhysResist);
            eEffects[nIndx++] = SetEffectEngineInteger(eEffects[0], EFFECT_INTEGER_VFX, nVFX); // VFX - looping glow

            eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_FIRE, fElemResist, PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_COLD, fElemResist, PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_ELEC, fElemResist);

            eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_NATURE, fElemResist, PROPERTY_ATTRIBUTE_DAMAGE_RESISTANCE_SPIRIT, fElemResist);

            eEffects[nIndx++] = EffectModifyProperty(PROPERTY_ATTRIBUTE_DEFENSE, fDefDec, PROPERTY_ATTRIBUTE_REGENERATION_STAMINA, fStamDec, PROPERTY_ATTRIBUTE_REGENERATION_STAMINA_COMBAT, fStamDec);
...
void _DeactivateModalAbility(object oCaster, int nAbility)
{
    ...
    if (nAbility == AF_ABILITY_STONEHEART && GetHasEffects(oCaster, AF_EFFECT_TYPE_STONE_WILL, AF_ABILITY_STONE_WILL)) {
        RemoveEffectsByParameters(oCaster, EFFECT_TYPE_INVALID, AF_ABILITY_STONE_WILL);
