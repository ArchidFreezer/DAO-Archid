...
void _ApplyImpact(struct EventSpellScriptImpactStruct stEvent) {
    ...
    effect eAoE = EffectAreaOfEffect(SHALE_BARRAGE, R"af_talent_shl_rock_barrage.ncs", 105418); // VFX - earth shaking
...
void main() {
    ...
    switch(nEventType) {
        ...
        case EVENT_TYPE_CUSTOM_EVENT_01: { // rock shower
            ...
            Engine_ApplyEffectAtLocation(EFFECT_DURATION_TYPE_INSTANT, eVFX, lLoc, 0.0, oCaster, AF_ABILITY_ROCK_BARRAGE);
            ...
            for(i = 0; i < nSize; i++) {
                ...
                // damage
                ApplyEffectOnObject(EFFECT_DURATION_TYPE_INSTANT, eDamage, oTargets[i], 0.0f, oCaster, AF_ABILITY_ROCK_BARRAGE);
                ...
                if (!IsImmuneToEffectType(oTargets[i], EFFECT_TYPE_KNOCKDOWN)) {
                    if (!ResistanceCheck(oCaster, oTargets[i], PROPERTY_ATTRIBUTE_STRENGTH, PROPERTY_ATTRIBUTE_RESISTANCE_PHYSICAL)) {
                        eEffect = EffectKnockdown(oCaster, 0, AF_ABILITY_ROCK_BARRAGE);
                        ...
                        ApplyEffectOnObject(EFFECT_DURATION_TYPE_TEMPORARY, eEffect, oTargets[i], RandomFloat(), oCaster, AF_ABILITY_ROCK_BARRAGE);
                        ...
                        eEffect = SetEffectEngineInteger(eEffect, EFFECT_INTEGER_USE_INTERPOLATION_ANGLE, 2);
                        eEffect = SetEffectEngineVector(eEffect, EFFECT_VECTOR_ORIGIN, GetPositionFromLocation(lLoc));
                        ApplyEffectOnObject(EFFECT_DURATION_TYPE_INSTANT, eEffect, oTargets[i], 0.0f, oCaster, AF_ABILITY_ROCK_BARRAGE);
                    }
                }

                if (IsObjectHostile(oCaster, oTargets[i])) {
                    // update impact threat each impact
                    AI_Threat_UpdateAbilityImpact(oTargets[i], oCaster, AF_ABILITY_ROCK_BARRAGE);
                }

                SendEventOnCastAt(oTargets[i], oCaster, AF_ABILITY_ROCK_BARRAGE, IsObjectHostile(oCaster, oTargets[i]));
            }
