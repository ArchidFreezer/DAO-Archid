...
const int AF_LOGGROUP_BLIZZARD = 5;
...
void main() {
    ...
    switch(nEventType) {
        case EVENT_TYPE_SPELLSCRIPT_CAST: {
            ...
            AF_LogInfo("EVENT_TYPE_SPELLSCRIPT_CAST", AF_LOGGROUP_BLIZZARD);
        ...
        case EVENT_TYPE_ENTER: {
            ...
            AF_LogInfo("Entering Blizzard: " + ToString(oTarget), AF_LOGGROUP_BLIZZARD);
        ...
        case EVENT_TYPE_AOE_HEARTBEAT: {
            ...
            AF_LogDebug("Blizzard Heartbeat " + ToString(nPhase), AF_LOGGROUP_BLIZZARD);

            // if the area is PRE tower 1
            object oArea = GetArea(OBJECT_SELF);
            string sTag = GetTag(oArea);
            int nType;
            AF_LogDebug("Blizzard Area = " + sTag, AF_LOGGROUP_BLIZZARD);
            ...
            if (sTag == AF_AR_PRE_TOWER_ISHAL_1) {
                ...
                if (IsLocationValid(lAoE)) AF_LogDebug("  AoE location valid.", AF_LOGGROUP_BLIZZARD);

                int nMax = GetArraySize(oVFX);
                AF_LogDebug("  There are " + ToString(nMax) + " VFX present.", AF_LOGGROUP_BLIZZARD);
                int nCount = 0;
                for (nCount = 0; nCount < nMax; nCount++) {
                    nType = GetEffectType(oVFX[nCount]);
                    sTag = ToString(nType);
                    AF_LogDebug("  Effect Type in AoE = " + sTag, AF_LOGGROUP_BLIZZARD);
                    if (nType == EFFECT_TYPE_VISUAL_EFFECT) {
                        AF_LogDebug("    Effect is a VFX.", AF_LOGGROUP_BLIZZARD);

                        // if the correct vfx type
                        nType = GetVisualEffectID(oVFX[nCount]);
                        AF_LogDebug("    Effect is ID " + ToString(nType) + " vs " + ToString(VFX_IMMOLATE_NO_CRUST), AF_LOGGROUP_BLIZZARD);
                        if (nType == VFX_IMMOLATE_NO_CRUST) {
                            // if close enough
                            location lLoc = GetVisualEffectLocation(oVFX[nCount]);
                            if (IsLocationValid(lLoc)) AF_LogDebug("    VFX location valid.", AF_LOGGROUP_BLIZZARD);

                            float fDistance = GetDistanceBetweenLocations(lAoE, lLoc);
                            AF_LogDebug("      Distance = " + ToString(fDistance), AF_LOGGROUP_BLIZZARD);
        ...
        case EVENT_TYPE_EXIT: {
            ...
            AF_LogInfo("Exiting Blizzard: " + ToString(oTarget), AF_LOGGROUP_BLIZZARD);
