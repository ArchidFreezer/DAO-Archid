    ...
    if(nType == EVENT_TYPE_SET_PLOT) // actions -> normal flags only
    {
        ...
        switch(nFlag)
        {
            ...
            case NTB_PHYLACTERY_ARCANE_WARRIOR_RELEASED:
            {
                ...
                if (nLeliana == TRUE)
                {
                    WR_SetPlotFlag(PLT_GENPT_APP_LELIANA, APP_LELIANA_INC_LOW, TRUE);
                }
                
                // Give the hero the Ivory Tower items
                if (!IsObjectValid(GetObjectByTag("af_chest_hvy_ivt"))) CreateItemOnObject(R"af_chest_hvy_ivt.uti", oPC, 1, "", TRUE);
                if (!IsObjectValid(GetObjectByTag("af_boot_hvy_ivt"))) CreateItemOnObject(R"af_boot_hvy_ivt.uti", oPC, 1, "", TRUE);
                if (!IsObjectValid(GetObjectByTag("af_glove_hvy_ivt"))) CreateItemOnObject(R"af_glove_hvy_ivt.uti", oPC, 1, "", TRUE);
                if (!IsObjectValid(GetObjectByTag("af_helm_hvy_ivt"))) CreateItemOnObject(R"af_helm_hvy_ivt.uti", oPC, 1, "", TRUE);
                if (!IsObjectValid(GetObjectByTag("af_dagg_nfb"))) CreateItemOnObject(R"af_dagg_nfb.uti", oPC, 1, "", TRUE);
                if (!IsObjectValid(GetObjectByTag("af_lsword_nfb"))) CreateItemOnObject(R"af_lsword_nfb.uti", oPC, 1, "", TRUE);
                if (!IsObjectValid(GetObjectByTag("af_shield_tow_nfb"))) CreateItemOnObject(R"af_shield_tow_nfb.uti", oPC, 1, "", TRUE);                
            ...
            case NTB_PHYLACTERY_BETRAYED:
            {
                ...
                if (nAlistair == TRUE)
                {
                    WR_SetPlotFlag(PLT_GENPT_APP_ALISTAIR, APP_ALISTAIR_DEC_MED, TRUE);
                }
                if (nLeliana == TRUE)
                {
                    WR_SetPlotFlag(PLT_GENPT_APP_LELIANA, APP_LELIANA_DEC_LOW, TRUE);
                }
                //Bad followers approve
                if (nMorrigan == TRUE)
                {
                    WR_SetPlotFlag(PLT_GENPT_APP_MORRIGAN, APP_MORRIGAN_INC_MED, TRUE);
                }
