    ...
    if(nType == EVENT_TYPE_SET_PLOT) // actions -> normal flags only
    {
        ...
        switch(nFlag)
        {
            ...
            case DWARF_MERCHANT_OPEN_STORE:
            {
                ...
                if (WR_GetPlotFlag("lot100pt_dwarf_merchant", 2) == TRUE) // "discount" = base price w/no mark up, by making priestess leave
                    SetStoreMarkUp(oStore, 100);
                if (WR_GetPlotFlag("lot100pt_dwarf_merchant", 0) == TRUE) // "double price" = 200% mark up, by persuading him to stay open
                    SetStoreMarkUp(oStore, 200);
                //threatening him to reduce price results in normal 160% sell price
