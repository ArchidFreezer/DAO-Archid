    ...
    else
    {
        ...
        switch(nFlag)
        {
            ...
            case BDC_DEFINED_PC_HAS_EVERDS_ARMOR_HELMET_EQUIPPED:
            {
                ...
                if (((GetTag(oPCArmor) == ResourceToTag(BDC_IM_EVERD_ARMOR_R)) ||(GetTag(oPCBite) == ResourceToTag(BDC_IM_EVERD_ARMOR_R))) &&
                    ((GetTag(oPCHelm)  == ResourceToTag(BDC_IM_EVERD_HELM_R)) ||(GetTag(oPCCloak)  == ResourceToTag(BDC_IM_EVERD_HELM_R))))
                {
                   bResult = TRUE;
                }
